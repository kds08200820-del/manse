/**
 * 3·1만세운동본부 — Cloudflare R2 Worker (갤러리 사진 + 임원 자료방 파일)
 * ---------------------------------------------------------------
 * 브라우저(정적 사이트)에서 R2로 직접 업로드하면 비밀키가 노출되므로,
 * 이 Worker가 중간에서 ①로그인·권한 검증 ②R2 저장 ③서빙을 담당합니다.
 *
 * ▣ 배포/갱신 방법 (Cloudflare 대시보드)
 *   1) Workers 및 Pages → (내 새 Worker) → Edit code → 이 파일 내용 전체 붙여넣기 → Deploy
 *   2) Settings 에 아래가 설정되어 있어야 합니다:
 *      · Variables and Secrets:
 *          SUPABASE_URL              = https://(내-새-프로젝트ID).supabase.co
 *          SUPABASE_ANON_KEY         = (내 anon publishable 키)
 *          ADMIN_EMAIL               = (최고관리자 이메일)
 *          SUPABASE_SERVICE_ROLE_KEY = (Supabase service_role 비밀키 · 임시비밀번호 기능용)
 *      · Bindings → R2 bucket:  Variable name = BUCKET,  R2 bucket = (내 새 R2 버킷)
 *
 * ▣ 엔드포인트
 *   GET  /i/<key>       사진 서빙 (갤러리)
...
export default {
  async fetch(request, env) {
    ...
    return new Response('만세운동본부 R2 Worker · OK', { headers: CORS });
  },
};
